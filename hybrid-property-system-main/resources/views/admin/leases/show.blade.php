    ...
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('admin.leases.edit', $lease) }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    @if($lease->status == 'terminated' && $lease->actual_end_date)
                        <div class="mt-4">
                            <label class="block text-sm font-medium text-gray-700">Actual End Date</label>
                            <p class="text-gray-900">{{ $lease->actual_end_date->format('M d, Y') }}</p>
                        </div>
                    @endif

                    @if($lease->status == 'terminated' && $lease->termination_reason)
                        <div class="mt-4">
                            <label class="block text-sm font-medium text-gray-700">Termination Reason</label>
                            <p class="text-gray-900 bg-red-50 p-3 rounded">{{ ucwords(str_replace('_', ' ', $lease->termination_reason)) }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    <div class="space-y-3">
                        @if($lease->document_path)
                            <a href="{{ route('admin.leases.download-document', $lease) }}"
...
<div id="renewModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form action="{{ route('admin.leases.renew', $lease) }}" method="POST">
                @csrf
...
<div id="terminateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form action="{{ route('admin.leases.terminate', $lease) }}" method="POST">
                @csrf
